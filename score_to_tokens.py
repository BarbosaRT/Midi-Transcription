...
def MusicXML_to_tokens(soup_or_mxml_path, note_name=True): # use this method
    ...
    tks = []
    if len(parts) == 1:
        tks = ['R'] + measures_to_tokens(parts[0], soup, staff=1, note_name=note_name)
        tks += ['L'] + measures_to_tokens(parts[0], soup, staff=2, note_name=note_name)
    elif len(parts) == 2:
        tks = ['R'] + measures_to_tokens(parts[0], soup, note_name=note_name)
        tks += ['L'] + measures_to_tokens(parts[1], soup, note_name=note_name)
    else:
